 ...
 * Review PDF Viewer — standalone script
 *
 * Requires:
 *   - pdfjsLib (pdf.js 3.11)
 *   - window.RPV_CONFIG = { pdfUrl, reviewId, apiBase, reviewerName }
 *
 * Fixes:
 *   - Highlight offset: gunakan stageRect.getBoundingClientRect() langsung
 *   - Underline: y = (rect.y + rect.h - strokeH/2) * s  (tidak terlalu ke atas)
 *   - Strikethrough: y tengah yang akurat
 *   - Sticky delete: animasi fade + immediate DOM remove
 *   - Fullscreen: toggle class is-fullscreen pada outer wrap
 *   - No text tool (dihapus), tambah brush tool
 *   - Mobile: bottom sheet + FAB
 *   - jsPDF: pakai versi UMD yang benar
 */
(function () {
    'use strict';
    ...
    function start() {
        ...
        const exportOverlay = document.getElementById('rpv-export-overlay');
        ...
        /* ── SANITIZER ───────────────────────── */
        const VT = ['highlight', 'underline', 'strikethrough', 'freehand', 'brush', 'comment', 'sticky', 'shape'];
        const VC = ['yellow', 'green', 'red', 'blue', 'orange', 'black', 'white', 'pink', 'purple', 'cyan'];
        const VS = ['rect', 'ellipse', 'arrow', 'line'];

        function sanitize(raw) {
            // brush simpan sebagai freehand di server (tipe sama, stroke_width berbeda)
        ...
        async function apiLoad() {
            if (!API) return [];
            try { const r = await fetch(API, { credentials: 'same-origin', headers: { 'Accept': 'application/json', 'X-Requested-With': 'XMLHttpRequest' } }); if (!r.ok) throw new Error(r.status); const j = await r.json(); return Array.isArray(j.data) ? j.data : []; }
            catch (e) { console.error('[RPV] load:', e); return []; }
        }
        async function apiSave(payload) {
            if (!API) { snack('⚠️ Simpan draft review dulu!', '#F59E0B'); return null; }
            const clean = sanitize(payload); showSync('Menyimpan...');
            try { const r = await fetch(API, { method: 'POST', credentials: 'same-origin', headers: hdrs(), body: JSON.stringify(clean) }); const j = await r.json(); if (!r.ok) { showSync('Gagal: ' + (j.message || r.status)); return null; } showSync('Tersimpan ✓', true); return j.data || null; }
        ...
        function doRender() {
            ...
            syncFC(); if (freeCtx) freeCtx.clearRect(0, 0, freeCanvas.width, freeCanvas.height);
        ...
        /* FIX: underline — garis di bagian BAWAH teks, bukan atas */
        function rUL(a, s) {
            if (!a.rect) return;
            const el = document.createElement('div'); el.dataset.annotId = String(a.id);
            const t = Math.max(1.5, 2 * s);
            /* y = bawah rect - setengah tebal garis — TIDAK dikurangi seluruh t */
        ...
        /* FIX: strikethrough — tepat di tengah teks */
        function rST(a, s) {
            if (!a.rect) return;
            const el = document.createElement('div'); el.dataset.annotId = String(a.id);
            const t = Math.max(1.5, 2 * s);
        ...
        function rSticky(a, s) {
            ...
            note.innerHTML = `<div class="rpv-sn-header"><span>📌</span><button type="button" class="rpv-sn-del">×</button></div><div class="rpv-sn-body">${esc(a.comment)}</div>`;
        ...
        /* FIX: sticky delete — animasi fade lalu hapus DOM langsung */
        function removeStickyAnimated(noteEl, id) {
            noteEl.classList.add('removing');
            setTimeout(async () => {
                noteEl.remove();
                await removeAnnot(id);
            }, 180);
        }
        ...
        function makeDraggable(el, annotData, s) {
            ...
            function onDown(e) { if (e.target.classList.contains('rpv-sn-del') || e.target.classList.contains('rpv-sn-body')) return; dragging = true; moved = false; const src = e.touches?.[0] ?? e; ox = src.clientX - el.offsetLeft; oy = src.clientY - el.offsetTop; el.style.zIndex = '20'; e.stopPropagation(); if (e.cancelable) e.preventDefault(); }
        ...
        function buildPanel() {
            ...
            [...annots].sort((a, b) => a.page - b.page || a.id - b.id).forEach(a => {
                ...
                el.innerHTML = `<div class="rpv-panel-dot" style="background:${hex(a.color)}"></div><div class="rpv-panel-body"><span class="rpv-panel-type">${ic[a.type] || '•'} ${a.type}</span><span class="rpv-panel-pg">Hal.${a.page}</span><div class="rpv-panel-text">${esc(a.comment || a.selected_text || a.shape_type || '—')}</div></div><button type="button" class="rpv-panel-del">🗑</button>`;
        ...
        function setTool(tool) {
            ...
            const LABELS = { pan: '🖐 Hand', select: '↖ Pilih', highlight: '✏️ Highlight', underline: '__ Underline', strikethrough: '~~ Strikethrough', comment: '💬 Komentar', freehand: '🖊 Pen Bebas', brush: '🖌️ Brush', shape: '⬛ Shape', eraser: '🧹 Hapus' };
            const lbl = document.getElementById('rpv-active-label'); if (lbl) lbl.textContent = LABELS[tool] || tool;
            const showSize = ['freehand', 'brush', 'shape'].includes(tool);
            document.getElementById('rpv-sizes')?.style.setProperty('display', showSize ? 'flex' : 'none');
        ...
        document.querySelectorAll('.rpv-tool[data-tool]').forEach(btn => {
            btn.addEventListener('click', () => { document.querySelectorAll('.rpv-tool[data-tool]').forEach(b => b.classList.remove('active')); btn.classList.add('active'); setTool(btn.dataset.tool); });
        });
        ...
        document.querySelectorAll('.rpv-size').forEach(d => { d.addEventListener('click', () => { document.querySelectorAll('.rpv-size').forEach(x => x.classList.remove('selected')); d.classList.add('selected'); activeSize = +d.dataset.size; brushSize = +d.dataset.size; }); });
        ...
        /* ── FREEHAND / BRUSH ────────────────── */
        function getFHSize() { return activeTool === 'brush' ? Math.max(4, brushSize * 3) : activeSize; }
        ...
        function fhMove(e) { if (!isDrawing || (activeTool !== 'freehand' && activeTool !== 'brush')) return; if (e.cancelable) e.preventDefault(); const p = stageXY(e), s = baseScale * zoomFactor; freePoints.push([p.x / s, p.y / s]); if (!freeCtx || freePoints.length < 2) return; const last = freePoints[freePoints.length - 2], cur = freePoints[freePoints.length - 1]; freeCtx.save(); freeCtx.strokeStyle = hex(activeColor); freeCtx.lineWidth = getFHSize() * s; freeCtx.lineCap = 'round'; freeCtx.lineJoin = 'round'; freeCtx.globalAlpha = activeTool === 'brush' ? .6 : .92; freeCtx.beginPath(); freeCtx.moveTo(last[0] * s, last[1] * s); freeCtx.lineTo(cur[0] * s, cur[1] * s); freeCtx.stroke(); freeCtx.restore(); }
        ...
        stage.addEventListener('click', e => {
            ...
            if (activeTool === 'sticky') { if (hitAnnot) return; if (e.target.closest('.rpv-popup,#rpv-annot-bar')) return; const p = stageXY(e), s = baseScale * zoomFactor; stickyPos = { x: p.x / s, y: p.y / s }; openPopup(document.getElementById('rpv-sticky-pop'), e.clientX, e.clientY); const t = document.getElementById('rpv-sticky-txt'); if (t) { t.value = ''; setTimeout(() => t.focus(), 30); } return; }
        ...
        async function exportPdfWithAnnotations() {
            ...
            if (!jsPDFLib) { snack('⚠️ Library PDF belum siap, coba lagi sebentar', '#F59E0B'); return; }
            exportInProgress = true;
            if (exportOverlay) exportOverlay.classList.add('show');
            snack('⏳ Menyiapkan PDF...', '#6366f1');
            try {
                const SCALE = 2.0;
                ...
                for (let p = 1; p <= pdfDoc.numPages; p++) {
                    ...
                    offCanvas.width = Math.floor(vp.width); offCanvas.height = Math.floor(vp.height);
                ...
                showSync('Export selesai ✓', true);
            } catch (err) { console.error('[RPV] export:', err); snack('❌ Gagal: ' + err.message, '#ef4444'); }
            finally { exportInProgress = false; if (exportOverlay) exportOverlay.classList.remove('show'); }
        }

        function renderAnnotToCanvas(c, a, s) {
            if (!a.rect && a.type !== 'freehand') return;
            c.save(); const col = hex(a.color);
            switch (a.type) {
        ...
        function closeSearch() { document.getElementById('rpv-search')?.classList.remove('show'); clearSearchHL(); currentQuery = ''; searchResults = []; searchIndex = -1; const i = document.getElementById('rpv-search-input'); if (i) i.value = ''; if (document.getElementById('rpv-search-results')) document.getElementById('rpv-search-results').innerHTML = ''; if (document.getElementById('rpv-search-status')) document.getElementById('rpv-search-status').textContent = 'Ketik untuk mencari...'; }
        ...
        function applySearchHL() {
            clearSearchHL(); if (!currentQuery || !pdfDoc) return;
            ...
            Array.from(textLayer.querySelectorAll('span')).forEach(span => {
                ...
                while (idx !== -1) {
                    try { const range = document.createRange(); range.setStart(span.firstChild, idx); range.setEnd(span.firstChild, Math.min(idx + q.length, text.length)); Array.from(range.getClientRects()).forEach(rect => { if (rect.width < 1 || rect.height < 1) return; const el = document.createElement('div'); el.className = 'rpvr-search-hl'; el.style.cssText = `position:absolute;left:${rect.left - sr.left}px;top:${rect.top - sr.top}px;width:${rect.width}px;height:${rect.height}px;`; annotLayer.appendChild(el); searchHighlights.push(el); }); } catch (_) { }
        ...
        async function doSearch(query) {
            if (!pdfDoc || !query.trim()) { clearSearchHL(); currentQuery = ''; if (document.getElementById('rpv-search-status')) document.getElementById('rpv-search-status').textContent = 'Ketik untuk mencari...'; return; }
            if (document.getElementById('rpv-search-status')) document.getElementById('rpv-search-status').textContent = 'Mencari...';
            ...
            if (!searchResults.length) { if (document.getElementById('rpv-search-status')) document.getElementById('rpv-search-status').textContent = `Tidak ditemukan: "${query}"`; clearSearchHL(); return; }
            if (document.getElementById('rpv-search-status')) document.getElementById('rpv-search-status').textContent = `${searchResults.length} hasil`; searchIndex = 0;
            searchResults.slice(0, 40).forEach((r, i) => { const el = document.createElement('div'); el.className = 'rpv-sri'; el.innerHTML = `<span class="pg">Hal.${r.page}</span><span>${esc(r.excerpt)}</span>`; el.addEventListener('click', () => { searchIndex = i; if (r.page !== pageNum) renderPage(r.page); else applySearchHL(); }); list.appendChild(el); });
        ...
        document.getElementById('rpv-snext')?.addEventListener('click', () => { if (!searchResults.length) return; searchIndex = (searchIndex + 1) % searchResults.length; const r = searchResults[searchIndex]; if (r.page !== pageNum) renderPage(r.page); else applySearchHL(); });
        document.getElementById('rpv-sprev')?.addEventListener('click', () => { if (!searchResults.length) return; searchIndex = (searchIndex - 1 + searchResults.length) % searchResults.length; const r = searchResults[searchIndex]; if (r.page !== pageNum) renderPage(r.page); else applySearchHL(); });
        ...
        function renderPage(num) {
            ...
            pdfDoc.getPage(num).then(async page => {
                ...
                scheduleRender();
                /* Update UI — gunakan optional chaining agar tidak error jika element null */
                ...
                // rpv-progress-txt is optional — tidak throw error jika tidak ada
                const ptEl = document.getElementById('rpv-progress-txt'); if (ptEl) ptEl.textContent = `Hal. ${num}/${pdfDoc?.numPages || '?'} · ${Math.round(pct)}%`;
        ...
        task.promise.then(async doc => {
            ...
            if (loadingEl) loadingEl.innerHTML = `<div style="font-size:2rem">⚠️</div><p style="color:#ef4444;font-weight:700;font-size:13px;">Gagal memuat PDF</p><p style="color:#6b7280;font-size:11px;">${err.message}</p>`;
        });
