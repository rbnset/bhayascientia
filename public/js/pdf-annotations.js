/**
 * pdf-annotations.js — v3.1 FINAL FIX
 * public/js/pdf-annotations.js
 *
 * PERBAIKAN v3.1:
 * - Eraser berfungsi (pointer-events fix, touch fix)
 * - Underline & Strikethrough berfungsi (type valid di controller)
 * - FAB hamburger toggle berfungsi
 * - annotLayer pointer-events diatur per-element, bukan global none
 * - Text tool berfungsi
 * - Semua anotasi tersimpan & di-load dari DB
 */
(function () {
    'use strict';

    /* ═══════════════════════════════════════════════════════════════
       WAIT FOR _pdfViewer + pdfDoc
    ═══════════════════════════════════════════════════════════════ */
    let _tick = 0;
    function waitForViewer(cb) {
        if (window._pdfViewer && window._pdfViewer.pdfDoc) { cb(window._pdfViewer); return; }
        if (_tick++ > 400) { console.error('[annot] timeout waiting for _pdfViewer'); return; }
    ...
    waitForViewer(function (V) {
        ...
        function colorHex(n) { return COLOR_MAP[n] || '#FFD700'; }
        ...
        function showSync(msg, ok = false) {
            ...
            syncTimer = setTimeout(() => syncEl.classList.remove('show'), ok ? 1800 : 4000);
        }
        ...
        async function apiLoad() {
            ...
            } catch (e) { console.error('[annot] load error:', e); return []; }
        }
        ...
        async function apiSave(payload) {
            ...
            try {
                ...
                if (!r.ok) {
                    console.error('[annot] save 422:', JSON.stringify(body));
                    showSync(`Gagal: ${body.message || r.status}`);
                    return null;
                }
                showSync('Tersimpan ✓', true);
                return body.data || null;
            } catch (e) { console.error('[annot] save error:', e); showSync('Error jaringan'); return null; }
        }
        ...
        async function apiDel(id) {
            ...
            } catch (e) { console.error('[annot] del error:', e); }
        }

        async function apiDelPage(page) {
            showSync('Membersihkan halaman...');
            try {
                await fetch(`${API}/page/${page}`, {
                    method: 'DELETE', credentials: 'same-origin', headers: apiHeaders(),
                });
                showSync('Halaman dibersihkan ✓', true);
            } catch (e) { console.error('[annot] delPage error:', e); }
        }
        ...
        async function loadAll() {
            ...
            console.log('[annot] loaded:', annots.length, 'annotations');
            scheduleRender();
            updateBadge();
            updateUndoRedo();
        }
        ...
        function scheduleRender() {
            ...
            renderPending = true;
            requestAnimationFrame(() => { renderPending = false; doRenderPage(); });
        }
        ...
        function doRenderPage() {
            ...
            /* 2. PENTING: annotLayer harus pointer-events:none secara default,
               tapi setiap elemen di dalamnya di-set pointer-events:auto sendiri */
            annotLayer.style.pointerEvents = 'none';
            annotLayer.style.overflow = 'hidden';

            /* 3. Clear freehand canvas */
            syncFreeCanvas();
            if (freeCtx) freeCtx.clearRect(0, 0, freeCanvas.width, freeCanvas.height);

            /* 4. Clear sticky notes & free text dari stage */
            stage.querySelectorAll('.sticky-note, .annot-freetext').forEach(e => e.remove());

            /* 5. Render anotasi halaman ini */
            annots.filter(a => a.page === page).forEach(a => {
                switch (a.type) {
                    ...
                    case 'comment': renderHighlight(a, scale); break;
        ...
        function renderHighlight(a, scale) {
            ...
            el.style.cssText = `
                ...
                background:${colorHex(a.color)}; opacity:${sel ? 0.75 : 0.38};
                border-radius:2px; pointer-events:auto; cursor:pointer; z-index:5;
                outline:${sel ? '2px solid #FF6B18' : 'none'};
                transition:opacity .12s;
            `;
            if (a.comment) {
                const dot = document.createElement('span');
                dot.style.cssText = 'position:absolute;top:-4px;right:-4px;width:8px;height:8px;background:#60A5FA;border-radius:50%;pointer-events:none;z-index:6;';
                el.appendChild(dot);
            }
            attachAnnotEvents(el, a);
        ...
        function renderUnderline(a, scale) {
            ...
            el.style.cssText = `
                ...
                top:${(a.rect.y + a.rect.h) * scale - Math.max(1.5, scale)}px;
                width:${a.rect.w * scale}px;
                height:${Math.max(1.5, 2 * scale)}px;
                background:${colorHex(a.color)};
                pointer-events:auto; cursor:pointer; z-index:5; opacity:0.9;
                border-radius:1px;
            `;
            attachAnnotEvents(el, a);
            annotLayer.appendChild(el);
        ...
        function renderFreehand(a, scale) {
            ...
            /* Hitbox agar bisa di-klik/erase */
            if (a.rect && (a.rect.w > 0 || a.rect.h > 0)) {
                const hit = document.createElement('div');
                hit.dataset.annotId = String(a.id);
                hit.style.cssText = `
                    position:absolute;
                    left:${(a.rect.x - 8) * scale}px; top:${(a.rect.y - 8) * scale}px;
                    width:${(a.rect.w + 16) * scale}px; height:${(a.rect.h + 16) * scale}px;
                    background:transparent; pointer-events:auto; cursor:pointer; z-index:6;
                `;
                attachAnnotEvents(hit, a);
        ...
        function renderShape(a, scale) {
            ...
            let svgBody = '';
            if (st === 'rect') {
                svgBody = `<rect x="${sw / 2}" y="${sw / 2}" width="${Math.max(1, w - sw)}" height="${Math.max(1, h - sw)}" rx="2" fill="none" stroke="${col}" stroke-width="${sw}"/>`;
            } else if (st === 'ellipse') {
                svgBody = `<ellipse cx="${w / 2}" cy="${h / 2}" rx="${Math.max(1, w / 2 - sw / 2)}" ry="${Math.max(1, h / 2 - sw / 2)}" fill="none" stroke="${col}" stroke-width="${sw}"/>`;
            } else if (st === 'arrow') {
                const hh = Math.max(4, h * 0.35), hx = Math.max(sw * 3, w * 0.25);
                svgBody = `<line x1="${sw}" y1="${h / 2}" x2="${w - hx + sw}" y2="${h / 2}" stroke="${col}" stroke-width="${sw}" stroke-linecap="round"/>
                           <polygon points="${w - sw / 2},${h / 2} ${w - hx},${h / 2 - hh} ${w - hx},${h / 2 + hh}" fill="${col}"/>`;
            } else if (st === 'line') {
                svgBody = `<line x1="${sw}" y1="${h / 2}" x2="${w - sw}" y2="${h / 2}" stroke="${col}" stroke-width="${sw}" stroke-linecap="round"/>`;
            }

            wrap.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" style="overflow:visible;display:block;pointer-events:none">${svgBody}</svg>`;
            attachAnnotEvents(wrap, a);
        ...
        function renderSticky(a, scale) {
            ...
            note.innerHTML = `
                <div class="sn-header"><span>📌</span><button class="sn-del">×</button></div>
        ...
        /* ── ATTACH EVENTS (click + touch) ke elemen annotLayer ── */
        function attachAnnotEvents(el, a) {
            el.addEventListener('click', ev => {
                ev.stopPropagation();
                if (activeTool === 'eraser') { removeAnnot(a.id); return; }
                if (activeTool === 'select') {
                    selectedId = selectedId == a.id ? null : String(a.id);
        ...
        function makeDraggable(el, annotData, scale) {
            ...
            function onUp() {
                ...
                if (idx >= 0 && annots[idx].rect) {
                    annots[idx].rect.x = parseFloat(el.style.left) / scale;
                    annots[idx].rect.y = parseFloat(el.style.top) / scale;
                }
            }
        ...
        function showTip(a, cx, cy) {
            ...
            let txt = `${icons[a.type] || '•'} ${a.type}`;
            if (a.comment) txt = `${icons[a.type] || '•'} ${a.comment.substring(0, 80)}`;
            else if (a.selected_text) txt = `${icons[a.type] || '•'} "${a.selected_text.substring(0, 60)}"`;
        ...
        document.addEventListener('click', e => {
            if (annotTip && !annotTip.contains(e.target) && !e.target.closest('[data-annot-id]')) {
                annotTip.classList.remove('show');
            }
        });
        ...
        async function doUndo() {
            ...
            if (op.action === 'add') {
                ...
                if (a) { await apiDel(a.id); annots = annots.filter(x => String(x.id) !== String(op.data.id)); redoStack.push({ action: 'del', data: a }); }
            } else {
                const saved = await apiSave(buildPayload(op.data));
        ...
        async function doRedo() {
            ...
            if (op.action === 'add') {
                ...
                const a = annots.find(x => String(x.id) === String(op.data.id));
                if (a) { await apiDel(a.id); annots = annots.filter(x => String(x.id) !== String(op.data.id)); undoStack.push({ action: 'del', data: a }); }
            }
            updateUndoRedo(); scheduleRender();
        }
        ...
        function buildPanel() {
            ...
            [...annots].sort((a, b) => a.page - b.page || a.id - b.id).forEach(a => {
                ...
                el.innerHTML = `
                    ...
                    <button class="ap-item-del">🗑</button>
                `;
        ...
        function setTool(tool) {
            ...
            /* Text layer: hanya aktif saat tool text-selection */
            ...
            /* Freehand canvas: aktif saat draw/shape/eraser */
            const needsDraw = ['freehand', 'shape'].includes(tool);
            if (freeCanvas) freeCanvas.style.pointerEvents = needsDraw ? 'auto' : 'none';
        ...
        function onSelEnd(e) {
            ...
            selTimer = setTimeout(async () => {
                ...
                const base = {
                    page: V.pageNum, color: activeColor,
        ...
        ═══════════════════════════════════════════════════════════ */
        if (freeCanvas) {
            freeCanvas.addEventListener('mousedown', fhStart, { passive: false });
            freeCanvas.addEventListener('mousemove', fhMove, { passive: false });
            freeCanvas.addEventListener('mouseup', fhEnd, { passive: false });
            freeCanvas.addEventListener('mouseleave', fhEnd, { passive: false });
            freeCanvas.addEventListener('touchstart', fhStart, { passive: false });
            freeCanvas.addEventListener('touchmove', fhMove, { passive: false });
            freeCanvas.addEventListener('touchend', fhEnd, { passive: false });
        }
        ...
        function fhMove(e) {
            ...
            freeCtx.lineWidth = activeSize * s;
            freeCtx.lineCap = 'round'; freeCtx.lineJoin = 'round'; freeCtx.globalAlpha = 0.92;
            freeCtx.beginPath();
            freeCtx.moveTo(last[0] * s, last[1] * s); freeCtx.lineTo(cur[0] * s, cur[1] * s);
        ...
        async function fhEnd(e) {
            ...
            await addAnnot({
                ...
                rect_x: bx, rect_y: by, rect_w: Math.max(...xs) - bx, rect_h: Math.max(...ys) - by,
            });
        }
        ...
        ═══════════════════════════════════════════════════════════ */
        if (freeCanvas) {
            freeCanvas.addEventListener('mousedown', shStart, { passive: false });
            freeCanvas.addEventListener('mousemove', shMove, { passive: false });
            freeCanvas.addEventListener('mouseup', shEnd, { passive: false });
            freeCanvas.addEventListener('touchstart', shStart, { passive: false });
            freeCanvas.addEventListener('touchmove', shMove, { passive: false });
            freeCanvas.addEventListener('touchend', shEnd, { passive: false });
        }
        ...
        function shStart(e) {
            ...
            shapePreviewEl.style.cssText = `position:absolute;pointer-events:none;z-index:25;
                border:${activeSize}px solid ${colorHex(activeColor)};
                ${activeShape === 'ellipse' ? 'border-radius:50%;' : ''}
                left:${drawStart.x}px;top:${drawStart.y}px;width:0;height:0;`;
        ...
        function shMove(e) {
            ...
            if (e.cancelable) e.preventDefault();
            const c = stageXY(e);
            Object.assign(shapePreviewEl.style, {
                left: Math.min(drawStart.x, c.x) + 'px', top: Math.min(drawStart.y, c.y) + 'px',
                width: Math.abs(c.x - drawStart.x) + 'px', height: Math.abs(c.y - drawStart.y) + 'px',
            });
        }
        async function shEnd(e) {
            ...
            await addAnnot({
                ...
                rect_x: x, rect_y: y, rect_w: w, rect_h: activeShape === 'line' ? 1 : h,
            });
        }

        /* ═══════════════════════════════════════════════════════════
           ERASER
           KUNCI: annotLayer.style.pointerEvents = 'none' (default),
           tapi setiap child el punya pointer-events:auto sendiri.
           Sehingga click event pada child bisa ditangkap.
        ═══════════════════════════════════════════════════════════ */
        /* Eraser cursor follow mouse */
        document.addEventListener('mousemove', e => {
            if (!eraserCur) return;
            if (activeTool === 'eraser') {
                eraserCur.style.display = 'block';
                eraserCur.style.left = e.clientX + 'px';
                eraserCur.style.top = e.clientY + 'px';
            } else {
                eraserCur.style.display = 'none';
            }
        });

        /* Eraser click: annotLayer child events sudah di-handle oleh attachAnnotEvents */
        /* Tambahan: klik di stage saat eraser tapi tidak kena elemen → tidak ada aksi */
        stage.addEventListener('click', e => {
            if (activeTool !== 'eraser') return;
            /* Jika klik tidak kena anotasi → tampilkan hint */
            if (!e.target.closest('[data-annot-id]')) {
                snack('Klik / sentuh anotasi untuk menghapus', '#60A5FA');
            }
        });
        ...
        stage.addEventListener('click', e => {
            ...
            if (e.target.closest('.sticky-note,.annot-freetext,#comment-popup,#sticky-popup,#freetext-popup,#annot-bottom-bar,[data-annot-id]')) return;
            const p = stageXY(e), s = V.getScale();
        ...
        stage.addEventListener('click', e => {
            ...
            if (e.target.closest('.annot-freetext,.sticky-note,#freetext-popup,#annot-bottom-bar,[data-annot-id]')) return;
            const p = stageXY(e), s = V.getScale();
        ...
        function ensureTextPopup() {
            ...
            p.innerHTML = `
                ...
                <textarea id="freetext-input" placeholder="Ketik teks..." style="width:100%;background:#2d2d2d;border:1.5px solid #3d3d3d;color:white;border-radius:8px;padding:.5rem;font-size:13px;resize:none;outline:none;height:60px;display:block;"></textarea>
                <div style="display:flex;gap:.4rem;margin-top:.5rem">
                    <button id="freetext-save" style="flex:1;padding:.45rem;background:#ff6b18;color:white;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">Tambah</button>
            ...
            document.getElementById('freetext-save').addEventListener('click', async () => {
                ...
                await addAnnot({
                    page: V.pageNum, type: 'text', color: activeColor, stroke_width: activeSize || 14,
                    rect_x: textPos.x, rect_y: textPos.y, rect_w: 200, rect_h: 30, comment: txt,
                });
        ...
        stage.addEventListener('click', e => {
            ...
            if (!e.target.closest('[data-annot-id],.sticky-note,.annot-freetext')) { selectedId = null; scheduleRender(); }
        });
        document.addEventListener('keydown', e => {
            if (['INPUT', 'TEXTAREA'].includes(e.target.tagName)) return;
            if ((e.key === 'Delete' || e.key === 'Backspace') && selectedId) {
                removeAnnot(selectedId); selectedId = null;
            }
        });
        ...
        if (mainCanvas) {
            new MutationObserver(() => {
                ...
                zoomTimer = setTimeout(() => { syncFreeCanvas(); scheduleRender(); }, 60);
        ...
        console.log('[annot] v3.1 loaded, slug=', slug);
