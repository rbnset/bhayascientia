...
<header class="mt-6">
    <div class="px-4 sm:px-6 lg:px-8 mx-auto max-w-[1130px]">
        <div class="flex items-center justify-between gap-4">
            <a href="{{ route('home') }}" class="flex items-center shrink-0 nav-hover-lift"
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route($ctaRoute) }}"
                    class="text-sm font-bold text-white xl:flex hidden h-[44px] shrink-0 items-center justify-center rounded-full bg-[#FF6B18] px-[18px]
                  transition-all duration-200 hover:-translate-y-[1px] hover:shadow-[0_10px_20px_0_#FF6B1880]
                  focus:outline-none focus-visible:ring-2 focus-visible:ring-[#FF6B18] focus-visible:ring-offset-2 focus-visible:ring-offset-[#F8F9FC]">
                    {{ $ctaLabel }}
                </a>
        ...
        {{-- MOBILE OVERLAY --}}
        <div id="mobileOverlay" class="fixed inset-0 z-40 hidden bg-black/25 xl:hidden" aria-hidden="true"></div>
        ...
        <div id="mobileMenu" class="relative z-50 hidden mt-4 xl:hidden">
            <div class="space-y-4 rounded-2xl bg-white p-3 border border-[#EEF0F7]">
                <div class="grid grid-cols-1 gap-2" aria-label="Menu utama versi mobile">
                    @foreach ($items as $item)
                    <a href="{{ route($item['route']) }}" class="js-mobile-item nav-mobile-item rounded-xl px-4 py-3 text-sm font-semibold border border-[#EEF0F7]
                      transition-all duration-200 hover:border-[#FF6B18]
                      focus:outline-none focus-visible:ring-2 focus-visible:ring-[#FF6B18] focus-visible:ring-offset-2 focus-visible:ring-offset-white
                      {{ request()->routeIs($item['route']) ? 'border-[#FF6B18] bg-[#FFF7F2] text-[#FF6B18]' : '' }}"
                    ...
                    <a href="{{ route($ctaRoute) }}"
                        class="js-mobile-item nav-mobile-item rounded-xl px-4 py-3 text-sm font-bold text-white bg-[#FF6B18] text-center
                    transition-all duration-200 hover:-translate-y-[1px] hover:shadow-[0_10px_20px_0_#FF6B1880]
                    focus:outline-none focus-visible:ring-2 focus-visible:ring-[#FF6B18] focus-visible:ring-offset-2 focus-visible:ring-offset-white"
