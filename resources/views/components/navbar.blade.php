...
{{-- Blade tidak punya macro, jadi kita buat partial inline dengan @php --}}
...
<header id="mainHeader" class="sticky top-0 z-50 transition-all duration-300" role="banner">
    <div class="px-4 sm:px-6 lg:px-8 mx-auto max-w-[1130px] relative">
        ...
        <div class="flex items-center justify-between gap-4 py-5">
            ...
            {{-- ===================== LOGO / AVATAR ===================== --}}
            @if($showAvatarWhenAuth && $authUser)
                ...
                <div class="relative" x-data="{ open: false }" @click.away="open = false">
                    <button @click="open = !open"
                        ...
                        <div class="relative">
                            ...
                            {{-- Upload manual --}}
                            <img src="{{ $authUser->photo_url }}" alt="{{ $authUser->name }}"
                                class="h-10 w-10 object-cover rounded-full border-2 border-[#FF6B18] shadow-sm group-hover:shadow-md transition-all">
                            @else
                            {{-- Fallback initials --}}
                            <div
                                ...
                                <span class="text-sm font-bold text-white">{{ $authUser->initials }}</span>
                            </div>
                            @endif
                            <span
                    ...
                    <button @click="open = !open"
                        ...
                        <div class="relative flex-shrink-0">
                            ...
                            <div
                                ...
                                <span class="text-base font-bold text-white sm:text-lg">{{ $authUser->initials }}</span>
                            </div>
                            @endif
                            <span
                                class="absolute bottom-0 right-0 w-2.5 h-2.5 bg-green-500 border-2 border-white rounded-full"></span>
    ...
    <div id="mobileMenu"
        ...
        <div class="rounded-2xl bg-white border border-[#EEF0F7] shadow-2xl overflow-hidden">
            ...
            <div class="p-4">
                ...
                {{-- User Card di Mobile Menu (jika login) --}}
                @if($showAvatarWhenAuth && $authUser)
                <div class="mb-4 p-4 bg-gradient-to-br from-[#FFF7F2] to-[#F8F9FC] rounded-xl border border-[#EEF0F7]">
                    <div class="flex items-center gap-3">
                        ...
                        <div
                            ...
                            <span class="text-lg font-bold text-white">{{ $authUser->initials }}</span>
                        </div>
                        @endif
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-bold text-[#1A1A1A] truncate">{{ $authUser->name }}</p>
                            <p class="text-xs text-[#737373] truncate">{{ $authUser->email }}</p>
                        </div>
                    </div>
                </div>
                @endif
                ...
                <div
                    ...
                    <ul class="space-y-2 text-xs text-[#737373]">
                        @foreach(['Browse ribuan publikasi gratis', 'Download PDF full article', 'Simpan ke perpustakaan
                        pribadi', 'Berlangganan newsletter gratis'] as $fitur)
