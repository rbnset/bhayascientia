    ...
    <div class="max-w-5xl px-4 mx-auto sm:px-6 lg:px-8">
        ...
        <div class="mb-6 bg-white rounded-2xl shadow-sm border border-[#EEF0F7] overflow-hidden">
            ...
            <div class="px-4 pb-5 sm:px-6">
                ...
                {{-- Baris 1: Avatar + Stats --}}
                <div class="flex items-end justify-between mb-3 -mt-9 sm:-mt-11">
                    {{-- Avatar --}}
                    <div class="relative flex-shrink-0">
                        <img src="{{ $user->profile_photo ? asset('storage/' . $user->profile_photo) : ($user->avatar ?: 'https://ui-avatars.com/api/?name=' . urlencode($user->name) . '&background=FF6B18&color=fff&size=128&bold=true') }}"
                            ...
                            class="object-cover w-[72px] h-[72px] sm:w-20 sm:h-20 bg-white border-4 border-white shadow-xl rounded-2xl">
                    ...
                    <div class="flex gap-2">
                        ...
                        <div class="text-center px-3 py-2 bg-[#FFF7F2] rounded-xl border border-[#FFE2D2] min-w-[58px]">
                            <div class="text-sm sm:text-lg font-black text-[#FF6B18]">{{ $stat['value'] }}</div>
                ...
                {{-- Baris 2: Nama + Email --}}
                <h2 class="text-base sm:text-lg font-black text-[#1A1A1A] leading-tight mb-0.5">
                    {{ $user->name }}
                </h2>
                <p class="text-xs sm:text-sm text-[#737373] mb-3">
                    {{ $user->email }}
                </p>

                {{-- Baris 3: Badges — masing-masing full info tidak terpotong --}}
                <div class="flex flex-col gap-1.5 sm:flex-row sm:flex-wrap sm:gap-2">
                    @if($user->job_title)
                    <span
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#FFF7F2] text-[#FF6B18] rounded-xl text-xs font-semibold border border-[#FFE2D2] w-fit">
                    ...
                    @if($user->affiliation)
                    <span
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#F8F9FC] text-[#555] rounded-xl text-xs font-semibold border border-[#EEF0F7] w-fit">
                        <svg class="w-3.5 h-3.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="https://orcid.org/{{ $user->orcid_id }}" target="_blank" rel="noopener noreferrer"
                        class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-[#F0FDF4] text-green-700 rounded-xl text-xs font-semibold border border-[#BBF7D0] hover:bg-[#DCFCE7] transition-colors w-fit">
                        <svg class="w-3.5 h-3.5 flex-shrink-0" viewBox="0 0 256 256" fill="none">
                        ...
                        <svg class="flex-shrink-0 w-3 h-3 text-green-600" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
                                d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
                                clip-rule="evenodd" />
                        </svg>
