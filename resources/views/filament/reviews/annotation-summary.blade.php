...
Dipakai di ViewReview infolist → Section "Anotasi PDF Reviewer"
Menampilkan ringkasan statistik + list anotasi terbaru
tanpa render PDF (ringan).
--}}
@php
...
'orange'=>'#FF6B18','black'=>'#374151','white'=>'#D1D5DB',
...
<div style="font-family: ui-sans-serif, system-ui, sans-serif;">
    ...
    <div style="display:flex; gap:.75rem; flex-wrap:wrap; margin-bottom:1rem;">
        ...
        {{-- Jenis terbanyak --}}
        @php $topType = $byType->sortByDesc(fn($g)=>$g->count())->keys()->first(); @endphp
        @if($topType)
        <div
            style="background:#f8fafc;border:1.5px solid #e2e8f0;border-radius:10px;padding:.75rem 1rem;min-width:100px;text-align:center;">
    ...
    <div style="margin-bottom:.75rem;">
        ...
        <div style="display:flex;flex-direction:column;gap:.4rem;max-height:320px;overflow-y:auto;padding-right:2px;">
            @foreach($annots->take(8) as $annot)
            @php
            $ti = $typeIcons[$annot->type] ?? ['•', $annot->type, '#9CA3AF', '#111'];
            $col = $COLORS[$annot->color] ?? '#9CA3AF';
            $txt = $annot->comment ?: ($annot->selected_text ? '"'.\Illuminate\Support\Str::limit($annot->selected_text,
            80).'"' : '—');
            @endphp
            <div
    ...
    {{-- ── Call to action ──────────────────────────── --}}
    <div
        style="background:linear-gradient(135deg,#fff7ed,#ffedd5);border:1.5px solid #fed7aa;border-radius:10px;padding:.875rem 1rem;display:flex;align-items:center;gap:.75rem;flex-wrap:wrap;">
        <svg style="width:20px;height:20px;color:#EA580C;flex-shrink:0;" fill="none" stroke="#EA580C"
            viewBox="0 0 24 24">
