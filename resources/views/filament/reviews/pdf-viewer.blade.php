...
Digunakan di Step 2 "Baca & Anotasi" oleh reviewer.

Variabel yang dikirim dari ReviewForm.php via Placeholder::content():
...
@php
// Semua variabel sudah dikirim dari ReviewForm.php
// $version, $publication, $slug, $pdfUrl
// Tidak perlu $get() di sini
@endphp
...
<div id="review-pdf-wrapper" class="overflow-hidden border border-gray-200 shadow-lg rounded-2xl dark:border-gray-700"
    ...
    <div id="rpv-toolbar" class="flex flex-wrap items-center gap-2 px-3 py-2 bg-gray-900 border-b border-gray-700">
        ...
        {{-- Color picker (static HTML — avoid @foreach inside rendered Blade string) --}}
        <div class="flex items-center gap-1">
...
/* ── CONFIG — variabel dikirim dari PHP via Placeholder::content() ── */
...
const COLORS = {
    ...
    blue:'#60A5FA',   orange:'#FF6B18', black:'#111',
    white:'#fff',     pink:'#F472B6',   purple:'#A78BFA', cyan:'#22D3EE'
};
...
async function apiSave(payload){
    try{
        const r=await fetch(API,{method:'POST',credentials:'same-origin',headers:hdrs(),body:JSON.stringify(sanitize(payload))});
        const j=await r.json();
        if(!r.ok){ console.error('[rpv] save',j); return null; }
        return j.data || null;
    }catch(e){ console.error('[rpv] save net',e); return null; }
}
...
async function apiDel(id){
    try{ await fetch(`${API}/${id}`,{method:'DELETE',credentials:'same-origin',headers:hdrs()}); }
...
function scheduleRender(){
    ...
    renderPending=true;
    requestAnimationFrame(()=>{ renderPending=false; renderAnnots(); });
}
...
function rSticky(a,s){
    if(!a.rect) return;
    const note=document.createElement('div'); note.className='rpv-sticky'; note.dataset.annotId=String(a.id);
    ...
    note.addEventListener('click',ev=>{
        if(activeTool==='eraser'){ ev.stopPropagation(); removeAnnot(a.id); }
        else { ev.stopPropagation(); showTip(a,ev.clientX,ev.clientY); }
    });
...
function rText(a,s){
    ...
    const el=document.createElement('div'); el.className='rpv-freetext'; el.dataset.annotId=String(a.id);
...
function attachEv(el,a){
    el.addEventListener('click',ev=>{
        ev.stopPropagation();
        if(activeTool==='eraser') removeAnnot(a.id);
        else showTip(a,ev.clientX,ev.clientY);
    });
}
...
function makeDraggable(el,annotData,s){
    ...
    el.addEventListener('mousedown',e=>{
        if(e.target.classList.contains('rpv-sticky-del')) return;
        dragging=true; moved=false;
        ox=e.clientX-el.offsetLeft; oy=e.clientY-el.offsetTop;
        el.style.zIndex='20'; e.stopPropagation();
    });
    ...
    document.addEventListener('mouseup',async()=>{
        ...
        await apiPatch(annotData.id,{rect_x:nx,rect_y:ny,rect_w:annotData.rect?.w||160,rect_h:annotData.rect?.h||80});
    });
}
...
document.getElementById('rpv-tip-del').addEventListener('click',async()=>{
    const id=tipText?.dataset.annotId; tooltip.style.display='none';
    if(id) await removeAnnot(id);
});
document.addEventListener('click',e=>{
    if(!tooltip.contains(e.target)&&!e.target.closest('[data-annot-id],.rpv-sticky,.rpv-freetext'))
...
document.addEventListener('mouseup',e=>{
    ...
    selTimer=setTimeout(async()=>{
        ...
        if(activeTool==='highlight'){     await addAnnot({...base,type:'highlight'}); window.getSelection()?.removeAllRanges(); }
            ...
            showPopup(commentPop,info.br.left,info.br.bottom+8);
            document.getElementById('rpv-comment-txt').value='';
            document.getElementById('rpv-comment-txt').focus();
        }
    },80);
});

function showPopup(pop,cx,cy){
    const vw=window.innerWidth, vh=window.innerHeight, pw=284, ph=170;
    pop.style.left=Math.max(4,Math.min(cx-pw/2,vw-pw-4))+'px';
    pop.style.top=Math.max(4,(cy+ph>vh?cy-ph-8:cy))+'px';
...
document.getElementById('rpv-comment-cancel').addEventListener('click',()=>{
    commentPop.style.display='none'; pendingRect=null; pendingText=null;
...
freeCanvas.addEventListener('mousemove',e=>{
    ...
    freeCtx.save();
    freeCtx.strokeStyle=hex(activeColor); freeCtx.lineWidth=activeSize*s;
    freeCtx.lineCap='round'; freeCtx.lineJoin='round'; freeCtx.globalAlpha=.9;
...
async function finishFreehand(){
    if(!isDrawing||activeTool!=='freehand') return;
    isDrawing=false; if(freePoints.length<2) return;
...
freeCanvas.addEventListener('mouseup',async e=>{
    ...
    drawStart=null;
    if(w<4&&h<4) return;
    await addAnnot({page:pageNum,type:'shape',color:activeColor,shape_type:activeShape,stroke_width:activeSize,rect_x:x,rect_y:y,rect_w:w,rect_h:h});
});
...
stage.addEventListener('click',e=>{
    ...
    document.getElementById('rpv-sticky-txt').focus();
});
...
document.getElementById('rpv-sticky-cancel').addEventListener('click',()=>{
    stickyPop.style.display='none'; stickyPos=null;
});
...
async function renderTextLayerFn(page,viewport){
    ...
    content.items.forEach(item=>{
        ...
        span.textContent=item.str; span.style.fontSize=fontH+'px';
        span.style.left=tx[4]+'px'; span.style.top=(tx[5]-fontH)+'px';
            ...
            pageAnnots.forEach(a=>{
                switch(a.type){
                    ...
                    case 'freehand':
                        ...
                        break;
                    case 'shape':
                        if(a.rect){ const sw=(a.stroke_width||2)*scale,col=hex(a.color),st=a.shape_type||'rect'; offCtx.save(); offCtx.strokeStyle=col; offCtx.lineWidth=sw; offCtx.globalAlpha=.9; offCtx.beginPath(); if(st==='rect'){offCtx.strokeRect(a.rect.x*scale+sw/2,a.rect.y*scale+sw/2,a.rect.w*scale-sw,a.rect.h*scale-sw);}else if(st==='ellipse'){const cx=(a.rect.x+a.rect.w/2)*scale,cy=(a.rect.y+a.rect.h/2)*scale,rx=a.rect.w/2*scale-sw/2,ry=a.rect.h/2*scale-sw/2;offCtx.ellipse(cx,cy,Math.max(1,rx),Math.max(1,ry),0,0,Math.PI*2);offCtx.stroke();}offCtx.restore(); }
                        break;
                    case 'sticky': case 'text':
                        if(a.rect&&a.comment){ const fs=Math.max(10,(a.stroke_width||14))*scale; offCtx.save(); offCtx.font=`600 ${fs}px sans-serif`; offCtx.fillStyle=hex(a.color); offCtx.globalAlpha=.85; a.comment.split('\n').forEach((ln,i)=>offCtx.fillText(ln,a.rect.x*scale,(a.rect.y*scale+fs*(i+1)))); offCtx.restore(); }
                        break;
                }
            });
