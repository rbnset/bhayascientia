...
// Di Filament v3, $this di dalam View::make() adalah page component langsung.
...
$pdfUrl = $version->pdf_file_path ? route('manuscripts.view', $version) : null;
}
}

$annotApiBase = $reviewId ? url("/api/review-annotations/{$reviewId}") : null;
@endphp
...
<div id="rpv-outer-wrap">
    ...
    {{-- EMPTY STATE --}}
    <div
        style="display:flex;flex-direction:column;align-items:center;justify-content:center;gap:1rem;text-align:center;padding:3rem 2rem;min-height:400px;">
        ...
        <p style="color:#6B7280;font-size:.875rem;max-width:340px;line-height:1.5;margin:0;">
            Kembali ke step sebelumnya dan pilih versi publikasi yang akan direview. PDF dan alat anotasi akan muncul di
            sini.
        </p>
    ...
    <div
        ...
        <p style="color:#6B7280;font-size:.875rem;max-width:380px;line-height:1.5;margin:0;">
            Anotasi membutuhkan ID review. Simpan form ini sebagai draft terlebih dahulu, lalu buka kembali untuk mulai
            memberi anotasi.
        </p>
        <p style="color:#4B5563;font-size:11px;margin:0;">PDF: <strong style="color:#9CA3AF;">{{ $publicationTitle
                }}</strong></p>
    </div>
    ...
    <div id="rpv-toolbar">
        <span class="rpv-title" title="{{ $publicationTitle }}">
            📄 {{ Str::limit($publicationTitle ?? 'Naskah', 40) }}
        </span>
        ...
        {{-- Fullscreen --}}
        <button type="button" class="rpv-btn rpv-desktop-only" id="rpv-fs-btn" title="Fullscreen (F)">
            <svg style="width:13px;height:13px;" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div id="rpv-annot-bar">
        ...
        <div class="rpv-ab-tools">
            ...
            {{-- GROUP 3: Drawing --}}
            <button type="button" class="rpv-tool" data-tool="freehand" title="Pen bebas (tipis)">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                    style="width:16px;height:16px;">
                    <path d="M12 20h9" />
            ...
            <button type="button" class="rpv-tool" data-tool="brush" title="Brush (tebal, transparan)">
                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                    style="width:16px;height:16px;">
                    <path d="M9.06 11.9l8.07-8.06a2.85 2.85 0 114.03 4.03l-8.06 8.08" />
                    <path
                        d="M7.07 14.94c-1.66 0-3 1.35-3 3.02 0 1.33-2.5 1.52-2 2.02 1 1 2.48 1 3.5 1 1.66 0 3-1.34 3-3s-1.34-3.04-1.5-3.04z"
