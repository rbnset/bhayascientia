...
PDF Viewer + Annotation untuk Filament Review Form.
Digunakan di Step 2 "Tulis Review" oleh reviewer.

Anotasi disimpan ke DB via /api/annotations/{slug}
Export PDF anotasi bisa di-download dan di-upload di Step 3.
--}}
...
$version = $pvId
? \App\Models\PublicationVersion::with('publication')->find($pvId)
: null;
$publication = $version?->publication;
$slug = $publication?->slug;

/* URL PDF untuk dimuat oleh PDF.js */
$pdfUrl = $version?->pdf_file_path
? route('manuscripts.stream', $version) // route stream private
: null;

/* Slug untuk API anotasi */
...
<div id="review-pdf-wrapper" class="overflow-hidden border border-gray-200 shadow-lg rounded-2xl dark:border-gray-700"
    ...
    <div id="rpv-toolbar" class="flex flex-wrap items-center gap-2 px-3 py-2 bg-gray-900 border-b border-gray-700">
        ...
        {{-- Color picker --}}
        <div class="flex items-center gap-1">
            @foreach(['yellow'=>'#FFD700','green'=>'#4ADE80','red'=>'#EF4444','blue'=>'#60A5FA','orange'=>'#FF6B18'] as
            $name=>$color)
            <button class="rpv-color {{ $name === 'yellow' ? 'rpv-color-sel' : '' }}" data-color="{{ $name }}"
                style="background:{{ $color }}" title="{{ ucfirst($name) }}"></button>
            @endforeach
        </div>
...
function sanitize(raw){
    ...
    return{
        ...
        shape_type  : VALID_SHAPES.includes(raw.shape_type)?raw.shape_type:(type==='shape'?'rect':null),
...
async function apiDelPage(page){try{await fetch(`${API}/page/${page}`,{method:'DELETE',credentials:'same-origin',headers:hdrs()});}catch(e){console.error('[rpv] delPage',e);}}
...
function renderAnnots(){
    const page=pageNum, scale=getScale();
...
function rHL(a,s){
    ...
    el.className='rpv-hl';
    el.style.cssText=`left:${a.rect.x*s}px;top:${a.rect.y*s}px;width:${a.rect.w*s}px;height:${a.rect.h*s}px;background:${hex(a.color)};`;
    if(a.type==='comment'&&a.comment){const dot=document.createElement('span');dot.style.cssText='position:absolute;top:-4px;right:-4px;width:8px;height:8px;background:#60A5FA;border-radius:50%;pointer-events:none;';el.appendChild(dot);}
...
function rShape(a,s){
    ...
    wrap2.className='rpv-annot-shape';
    wrap2.style.cssText=`left:${x}px;top:${y}px;width:${w}px;height:${h}px;`;
...
function rSticky(a,s){
    if(!a.rect)return;
    const note=document.createElement('div');note.className='rpv-sticky';
    note.dataset.annotId=String(a.id);
    note.style.left=(a.rect.x*s)+'px';note.style.top=(a.rect.y*s)+'px';
    note.innerHTML=`<button class="rpv-sticky-del">×</button><div style="pointer-events:none;padding-right:16px;">${esc(a.comment)}</div>`;
    note.querySelector('.rpv-sticky-del').addEventListener('click',ev=>{ev.stopPropagation();removeAnnot(a.id);});
    note.addEventListener('click',ev=>{if(activeTool==='eraser'){ev.stopPropagation();removeAnnot(a.id);}else{ev.stopPropagation();showTip(a,ev.clientX,ev.clientY);}});
    makeDraggable(note,a,s);stage.appendChild(note);
}
function rText(a,s){
    if(!a.rect)return;
    const fontSize=Math.max(10,(a.stroke_width||14))*s;
    const el=document.createElement('div');el.className='rpv-freetext';
    el.dataset.annotId=String(a.id);
    el.style.cssText=`position:absolute;left:${a.rect.x*s}px;top:${a.rect.y*s}px;font-size:${fontSize}px;line-height:1.4;color:${hex(a.color)};pointer-events:auto;cursor:pointer;z-index:8;white-space:pre-wrap;word-break:break-word;max-width:${300*s}px;font-family:sans-serif;font-weight:600;user-select:none;`;
    el.textContent=a.comment||'';
    el.addEventListener('click',ev=>{if(activeTool==='eraser'){ev.stopPropagation();removeAnnot(a.id);}else{ev.stopPropagation();showTip(a,ev.clientX,ev.clientY);}});
    stage.appendChild(el);
}

function attachEv(el,a){
    el.addEventListener('click',ev=>{ev.stopPropagation();if(activeTool==='eraser'){removeAnnot(a.id);}else{showTip(a,ev.clientX,ev.clientY);}});
}

function makeDraggable(el,annotData,s){
    let ox=0,oy=0,dragging=false,moved=false;
    el.addEventListener('mousedown',e=>{if(e.target.classList.contains('rpv-sticky-del'))return;dragging=true;moved=false;ox=e.clientX-el.offsetLeft;oy=e.clientY-el.offsetTop;el.style.zIndex='20';e.stopPropagation();});
    document.addEventListener('mousemove',e=>{if(!dragging)return;moved=true;el.style.left=(e.clientX-ox)+'px';el.style.top=(e.clientY-oy)+'px';});
    document.addEventListener('mouseup',async()=>{if(!dragging)return;dragging=false;el.style.zIndex='9';if(!moved)return;const nx=parseFloat(el.style.left)/s,ny=parseFloat(el.style.top)/s;const idx=annots.findIndex(a=>String(a.id)===String(annotData.id));if(idx>=0&&annots[idx].rect){annots[idx].rect.x=nx;annots[idx].rect.y=ny;}await apiPatch(annotData.id,{rect_x:nx,rect_y:ny,rect_w:annotData.rect?.w||160,rect_h:annotData.rect?.h||80});});
}
...
function showTip(a,cx,cy){
    ...
    tipText.textContent=txt;tipText.dataset.annotId=String(a.id);
    tooltip.style.display='block';
    const vw=window.innerWidth,vh=window.innerHeight;
    tooltip.style.left=Math.max(4,Math.min(cx-135,vw-270))+'px';
...
async function addAnnot(payload){
    ...
    annots.push(saved);undoStack.push({action:'add',data:saved});redoStack=[];
...
async function removeAnnot(id){
    ...
    await apiDel(a.id);annots=annots.filter(x=>String(x.id)!==String(id));
    undoStack.push({action:'del',data:a});redoStack=[];updateUndoRedo();scheduleRender();
}

/* ── UNDO / REDO ── */
function updateUndoRedo(){
    document.getElementById('rpv-undo').disabled=!undoStack.length;
    document.getElementById('rpv-redo').disabled=!redoStack.length;
}
async function doUndo(){
    if(!undoStack.length)return;const op=undoStack.pop();
...
async function doRedo(){
    if(!redoStack.length)return;const op=redoStack.pop();
...
function updateCount(){
    ...
    const badge=document.getElementById('rpv-annot-count');
    const num=document.getElementById('rpv-annot-num');
    if(badge&&num){num.textContent=n;badge.style.display=n>0?'flex':'none';}
}

/* ── TOOL MANAGEMENT ── */
function setTool(tool){
    activeTool=tool;
    document.querySelectorAll('.rpv-tool-btn').forEach(b=>b.classList.toggle('rpv-active',b.dataset.tool===tool));
    ...
    freeCanvas.style.pointerEvents=['freehand','shape'].includes(tool)?'auto':'none';
}
...
document.querySelectorAll('.rpv-color').forEach(sw=>{
    sw.addEventListener('click',()=>{
        document.querySelectorAll('.rpv-color').forEach(s=>s.classList.remove('rpv-color-sel'));
        sw.classList.add('rpv-color-sel');activeColor=sw.dataset.color;
    });
});
...
function getSelInfo(){
    ...
    const sr=stage.getBoundingClientRect(),s=getScale();
...
document.addEventListener('mouseup',e=>{
    if(e.target.closest('#rpv-comment-pop,#rpv-sticky-pop'))return;
    ...
    selTimer=setTimeout(async()=>{
        ...
        if(activeTool==='highlight'){await addAnnot({...base,type:'highlight'});window.getSelection()?.removeAllRanges();}
        else if(activeTool==='underline'){await addAnnot({...base,type:'underline'});window.getSelection()?.removeAllRanges();}
        else if(activeTool==='strikethrough'){await addAnnot({...base,type:'strikethrough'});window.getSelection()?.removeAllRanges();}
        else if(activeTool==='comment'){pendingRect=info.rect;pendingText=info.text;showPopup(commentPop,info.br.left,info.br.bottom+8);document.getElementById('rpv-comment-txt').value='';document.getElementById('rpv-comment-txt').focus();}
    },80);
});
...
document.getElementById('rpv-comment-save').addEventListener('click',async()=>{
    const txt=document.getElementById('rpv-comment-txt').value.trim();if(!txt||!pendingRect)return;
    ...
    window.getSelection()?.removeAllRanges();pendingRect=null;pendingText=null;
});
...
freeCanvas.addEventListener('mousemove',e=>{
    if(!isDrawing||activeTool!=='freehand')return;const p=stageXY(e),s=getScale();freePoints.push([p.x/s,p.y/s]);
    if(freePoints.length<2)return;const last=freePoints[freePoints.length-2],cur=freePoints[freePoints.length-1];
    freeCtx.save();freeCtx.strokeStyle=hex(activeColor);freeCtx.lineWidth=activeSize*s;freeCtx.lineCap='round';freeCtx.lineJoin='round';freeCtx.globalAlpha=.9;
    freeCtx.beginPath();freeCtx.moveTo(last[0]*s,last[1]*s);freeCtx.lineTo(cur[0]*s,cur[1]*s);freeCtx.stroke();freeCtx.restore();
});
freeCanvas.addEventListener('mouseup',async e=>{
    if(!isDrawing||activeTool!=='freehand')return;isDrawing=false;if(freePoints.length<2)return;
    const xs=freePoints.map(p=>p[0]),ys=freePoints.map(p=>p[1]),bx=Math.min(...xs),by=Math.min(...ys);
    await addAnnot({page:pageNum,type:'freehand',color:activeColor,stroke_width:activeSize,path_points:freePoints,rect_x:bx,rect_y:by,rect_w:Math.max(...xs)-bx,rect_h:Math.max(...ys)-by});
});
freeCanvas.addEventListener('mouseleave',async e=>{if(!isDrawing||activeTool!=='freehand')return;isDrawing=false;if(freePoints.length<2)return;const xs=freePoints.map(p=>p[0]),ys=freePoints.map(p=>p[1]),bx=Math.min(...xs),by=Math.min(...ys);await addAnnot({page:pageNum,type:'freehand',color:activeColor,stroke_width:activeSize,path_points:freePoints,rect_x:bx,rect_y:by,rect_w:Math.max(...xs)-bx,rect_h:Math.max(...ys)-by});});
...
stage.addEventListener('click',e=>{
    ...
    if(e.target.closest('[data-annot-id],.rpv-sticky,.rpv-freetext,#rpv-comment-pop,#rpv-sticky-pop'))return;
    const p=stageXY(e),s=getScale();stickyPos={x:p.x/s,y:p.y/s};
    showPopup(stickyPop,e.clientX,e.clientY);
    document.getElementById('rpv-sticky-txt').value='';document.getElementById('rpv-sticky-txt').focus();
});
document.getElementById('rpv-sticky-save').addEventListener('click',async()=>{
    const txt=document.getElementById('rpv-sticky-txt').value.trim();if(!txt||!stickyPos)return;
    stickyPop.style.display='none';
    await addAnnot({page:pageNum,type:'sticky',color:activeColor,rect_x:stickyPos.x,rect_y:stickyPos.y,rect_w:160,rect_h:80,comment:txt});
    stickyPos=null;
});
...
pdfjsLib.getDocument({url:PDF_URL,withCredentials:true,verbosity:0}).promise.then(async doc=>{
    ...
    loadingEl.innerHTML='<p style="color:#f87171;font-weight:600;">Gagal memuat PDF. Pastikan file tersedia.</p>';
    console.error('[rpv] pdf load',e);
});
...
/* ════════════════════════════════════════════════════════════
   EXPORT PDF + ANOTASI
   Menggunakan PDF.js + html-to-canvas approach:
   Render setiap halaman ke canvas, gambar ulang anotasi di atasnya,
   lalu gabungkan dengan jsPDF.
════════════════════════════════════════════════════════════ */
document.getElementById('rpv-export').addEventListener('click',async()=>{
    ...
    try{
        /* Load jsPDF jika belum ada */
        if(!window.jspdf){
            await loadScript('https://cdnjs.cloudflare.com/ajax/libs/jspdf/2.5.1/jspdf.umd.min.js');
        }
        const {jsPDF}=window.jspdf;

        /* Group anotasi per halaman */
        const annotByPage={};
        ...
        for(let p=1;p<=pdfDoc.numPages;p++){
            ...
            const viewport=page.getViewport({scale:2}); /* scale 2 untuk kualitas cetak */

            /* Render PDF page ke offscreen canvas */
            const offCanvas=document.createElement('canvas');
            offCanvas.width=Math.floor(viewport.width);offCanvas.height=Math.floor(viewport.height);
            const offCtx=offCanvas.getContext('2d');
            await page.render({canvasContext:offCtx,viewport}).promise;

            /* Gambar anotasi di atas canvas */
            const pageAnnots=annotByPage[p]||[];
            const scale=viewport.scale;

            pageAnnots.forEach(a=>{
                switch(a.type){
                    case'highlight':case'comment':
                        if(a.rect){offCtx.save();offCtx.globalAlpha=.38;offCtx.fillStyle=hex(a.color);offCtx.fillRect(a.rect.x*scale,a.rect.y*scale,a.rect.w*scale,a.rect.h*scale);offCtx.restore();}
                        break;
                    case'underline':
                        if(a.rect){offCtx.save();offCtx.globalAlpha=.9;offCtx.fillStyle=hex(a.color);offCtx.fillRect(a.rect.x*scale,(a.rect.y+a.rect.h)*scale-2,a.rect.w*scale,2);offCtx.restore();}
                        break;
                    case'strikethrough':
                        if(a.rect){offCtx.save();offCtx.globalAlpha=.9;offCtx.fillStyle=hex(a.color);offCtx.fillRect(a.rect.x*scale,(a.rect.y+a.rect.h/2)*scale-1,a.rect.w*scale,2);offCtx.restore();}
                        break;
                    case'freehand':
                        if(a.path_points?.length){offCtx.save();offCtx.strokeStyle=hex(a.color);offCtx.lineWidth=(a.stroke_width||2)*scale;offCtx.lineCap='round';offCtx.lineJoin='round';offCtx.globalAlpha=.9;offCtx.beginPath();offCtx.moveTo(a.path_points[0][0]*scale,a.path_points[0][1]*scale);for(let i=1;i<a.path_points.length;i++)offCtx.lineTo(a.path_points[i][0]*scale,a.path_points[i][1]*scale);offCtx.stroke();offCtx.restore();}
                        break;
                    case'shape':
                        if(a.rect){
                            const sw=(a.stroke_width||2)*scale,col=hex(a.color),st=a.shape_type||'rect';
                            offCtx.save();offCtx.strokeStyle=col;offCtx.lineWidth=sw;offCtx.globalAlpha=.9;
                            offCtx.beginPath();
                            if(st==='rect'){offCtx.strokeRect(a.rect.x*scale+sw/2,a.rect.y*scale+sw/2,a.rect.w*scale-sw,a.rect.h*scale-sw);}
                            else if(st==='ellipse'){const cx=(a.rect.x+a.rect.w/2)*scale,cy=(a.rect.y+a.rect.h/2)*scale,rx=a.rect.w/2*scale-sw/2,ry=a.rect.h/2*scale-sw/2;offCtx.ellipse(cx,cy,Math.max(1,rx),Math.max(1,ry),0,0,Math.PI*2);offCtx.stroke();}
                            offCtx.restore();
                        }
                        break;
                    case'sticky':case'text':
                        if(a.rect&&a.comment){
                            const fs=Math.max(10,(a.stroke_width||14))*scale;
                            offCtx.save();offCtx.font=`600 ${fs}px sans-serif`;offCtx.fillStyle=hex(a.color);offCtx.globalAlpha=.85;
                            const lines=a.comment.split('\n');
                            lines.forEach((ln,i)=>offCtx.fillText(ln,a.rect.x*scale,(a.rect.y*scale+fs*(i+1))));
                            offCtx.restore();
                        }
                        break;
                }
            ...
            /* Tambahkan halaman ke jsPDF */
            const imgData=offCanvas.toDataURL('image/jpeg',0.92);
            const mmW=viewport.width/2.8346, mmH=viewport.height/2.8346; /* px to mm at 72dpi */

            if(!pdf){
                pdf=new jsPDF({orientation:viewport.width>viewport.height?'landscape':'portrait',unit:'mm',format:[mmW,mmH]});
            }else{
                pdf.addPage([mmW,mmH],viewport.width>viewport.height?'landscape':'portrait');
            }
        ...
        console.error('[rpv] export error',err);
        alert('Gagal export PDF: '+err.message);
    }finally{
        btn.disabled=false;
        btn.innerHTML='<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg> Export PDF + Anotasi';
    }
});
